...
if os.path.exists("links.csv"):
    os.remove("links.csv")
if os.path.exists("bad_links.csv"):
    os.remove("bad_links.csv")
...
def append_list_as_row(file_name, list_of_elem):
    ...
    if os.path.exists("links.csv") :
        ...
        with open(file_name, 'a+', newline='') as first:
            ...
            # Add contents of list as last row in the csv file
            csv_writer.writerow(['linkfrom','linkto'])
...
def append_list_as_badrow(file_name, list_of_elem):
    ...
    if os.path.exists("bad_links.csv") :
        ...
        with open('bad_links.csv', 'w', newline='') as f:
            # Create a writer object from csv module
            csv_writer = writer(f)
...
for line in xopen :
    ...
    if line.startswith('INSERT INTO `link` VALUES ') :
    
        line=line.lstrip("INSERT INTO `link` VALUES (")
        ...
        for dum in line :
            ...
            if len(list)>3 :
                ...
                append_list_as_badrow('bad_links.csv',list)
            else   : 
                #Calling function and give CSV file name and value of list to stored in CSV file.
                append_list_as_row('links.csv',list)
