...
if os.path.exists("page_urn.csv"):
    os.remove("page_urn.csv")
if os.path.exists("bad_page_urn.csv"):
    os.remove("bad_page_urn.csv")
...
def append_list_as_row(file_name, list_of_elem):
    ...
    if os.path.exists("page_urn.csv") :
        ...
        with open('page_urn.csv', 'w', newline='') as f:
            # Create a writer object from csv module
            csv_writer = writer(f)
        ...
        with open(file_name, 'a+', newline='') as first:
            ...
            # Add contents of list as last row in the csv file
            csv_writer.writerow(['pagename','urn'])
...
def append_list_as_badrow(file_name, list_of_elem):
    ...
    if os.path.exists("bad_page_urn.csv") :
        ...
        with open('bad_page_urn.csv', 'w', newline='') as f:
            # Create a writer object from csv module
            csv_writer = writer(f)
...
for line in xopen :
    ...
    if line.startswith('INSERT INTO `page_urn` VALUES ') :
    
        line=line.lstrip("INSERT INTO `page_urn` VALUES (")
        ...
        for dum in line :
            ...
            if len(list)>3 :
                ...
                append_list_as_badrow('bad_page_urn.csv',list)
            else   : 
                #Calling function and give CSV file name and value of list to stored in CSV file.
                append_list_as_row('page_urn.csv',list)
