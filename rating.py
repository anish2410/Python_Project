...
if os.path.exists("bad_rating.csv"):
    os.remove("bad_rating.csv")
...
def append_list_as_row(file_name, list_of_elem):
    ...
    if os.path.exists("rating.csv") :
        ...
        with open(file_name, 'a+', newline='') as first:
            ...
            # Add contents of list as last row in the csv file
            csv_writer.writerow(['dimension','raterpage','rateepage','ratingvalue','rateeversion','tstamp'])
...
def append_list_as_badrow(file_name, list_of_elem):
    ...
    if os.path.exists("bad_rating.csv") :
        ...
        with open('bad_rating.csv', 'w', newline='') as f:
            # Create a writer object from csv module
            csv_writer = writer(f)
...
for line in xopen :
    ...
    if line.startswith('INSERT INTO `rating` VALUES ') :
    
        line=line.lstrip("INSERT INTO `rating` VALUES (")
        ...
        for dum in line :
            ...
            #checking length of list if list have more than 6 then stored as bad records and other stored as good records 
            if len(list)>7 :
                
                #Calling function and give CSV file name and value of list to stored in CSV file.
                append_list_as_badrow('bad_rating.csv',list)
            else   : 
                #Calling function and give CSV file name and value of list to stored in CSV file.
                append_list_as_row('rating.csv',list)
